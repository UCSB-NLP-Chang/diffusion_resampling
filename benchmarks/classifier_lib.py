...
def get_discriminator(latent_extractor_ckpt, discriminator_ckpt, condition,
        img_resolution=32, device='cuda', enable_grad=True, dropout=0.0):
    classifier = load_classifier(latent_extractor_ckpt, img_resolution, device, eval=True)
    discriminator = load_discriminator(discriminator_ckpt, device, condition, eval=True, dropout=dropout)
...
def load_discriminator(ckpt_path, device, condition, eval=False, channel=512, dropout=0.0):
...
def get_likelihood_ratio(discriminator, vpsde, input, time, class_labels, img_resolution):
    mean_vp_tau, tau = vpsde.transform_unnormalized_wve_to_normalized_vp(time) ## VP pretrained classifier
    input = mean_vp_tau.view(-1)[:,None,None,None] * input
    if img_resolution == 64: # ADM trained UNet classifier for 64x64 with Cosine VPSDE
        tau = vpsde.compute_t_cos_from_t_lin(tau)
    tau = torch.ones(input.shape[0], device=tau.device) * tau
    logits = discriminator(input.float(), timesteps=tau, condition=class_labels)
